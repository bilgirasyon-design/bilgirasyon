...
function injectBeforeArticle(html, breadcrumbHtml) {
  ...
  if (!html.includes(marker)) {
    throw new Error(
      "Production Build Engine: <article> bulunamadı."
    );
  }
...
function injectSchema(html, schema) {
  ...
  if (!html.includes(marker)) {
    throw new Error(
      "Production Build Engine: </head> bulunamadı."
    );
  }
...
async function run() {
  if (!(await pathExists(DIST_DIR))) {
    throw new Error(
      "Production Build Engine: dist bulunamadı. Önce build çalışmalıdır."
    );
  }

  const siteConfig = await readJson(
    SITE_CONFIG_FILE
  );

  const domain = siteConfig.site?.domain;
  const siteName =
    siteConfig.site?.name ?? "Bilgirasyon";

  if (!domain) {
    throw new Error(
      "Production Build Engine: site.domain bulunamadı."
    );
  }
  ...
  for (const record of contentIndex) {
    ...
    if (routeMap.has(route.route)) {
      throw new Error(
        `Production Build Engine: duplicate route: ${route.route}`
      );
    }
  ...
  for (const article of articles) {
    if (!(await pathExists(article.distPath))) {
      throw new Error(
        `Production Build Engine: HTML çıktısı bulunamadı: ${article.distPath}`
      );
    }
    ...
    if (
      !html.includes(
        'data-bilgirasyon-schema="breadcrumb"'
      )
    ) {
      html = injectSchema(
        html,
  ...
  if (brokenLinks.length > 0) {
    const details = brokenLinks
      .map(
        (item) =>
          `- ${item.source} -> ${item.href} (${item.route})`
      )
      .join("\n");
  ...
  console.log(
    `→ Breadcrumb schema: ${articles.length}`
  );
  console.log(
    `→ Orphan content: ${orphanContent.length}`
  );
}
